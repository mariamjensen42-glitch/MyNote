...
- 图片会真的显示出来：`![](assets/a.png)` 这类**仓库内相对路径**（按笔记所在目录解析，`/` 开头表示仓库根）走 SAF 读原图，`![](https://…)` 走网络取图；两者都做等比缩放与内存缓存，读不到就退化成 alt 文字而不是叉图
- 工具条的「图片」按钮改为**从系统相册选图**：图片被复制进仓库的附件目录（默认 `attachments/`，可在设置里改，第一次用会自动建目录），笔记里写入相对引用（如 `![](attachments/图.jpg)`）。原图不动，同名会加 ` 2`、` 3` 后缀而不是覆盖
...
- **只有独占一段的图片会渲染成图**，夹在文字中间的图片仍是 `[alt]` 文字（行内图片需要在文本里嵌占位符，还没做）。
