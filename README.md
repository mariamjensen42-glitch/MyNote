...
- 源码着色与预览同源：`~~删除线~~`、Setext 标题（`===`）、分隔线、行内代码、裸链接都会着色，行内代码里的 `**` 不会被误加粗，`snake_case` 不会被误当斜体；强调的定界符按 CommonMark 的左右侧规则判断，`2 * 3 * 4` 与 `foo__bar__baz` 是普通文字而不是斜体/粗体
- 预览可交互：点任务复选框直接勾掉（改写源码里那一行），点链接与裸链接交给系统打开；GFM 表格按对齐渲染
- 引用是容器：`>` 后面的内容按同一套规则解析，所以引用里可以有段落、列表、任务、标题、代码块，`> > 嵌套` 也能嵌套；连续几行 `> ` 属于同一个引用
- HTML 实体按规范解码（`&amp;` → `&`、`&lt;` → `<`、`&#65;` → `A`、`&nbsp;` 等），HTML 标签本身按 CommonMark 的做法原样显示为文字（预览不渲染 HTML，但也不吞掉它）
...
- 显示上次刷新时间与「N 处外部修改」—— 即被其他应用或同步工具改动过的文件数；**点这一行即可重新扫描**（别的应用刚写入的笔记不用重启就能看到）
- 新建文件夹 / 新建笔记
...
- **预览还不支持脚注与缩进式代码块**（要用 ``` 围栏）。解析器覆盖标题、段落、列表、任务、引用（含嵌套与引用内的块）、围栏代码、分隔线、GFM 表格（含对齐）、裸链接自动识别、行内与整段图片、HTML 实体；其他语法会降级成段落或原样文字，不会丢字。
- **HTML 标签原样显示，不渲染**：预览是纯文本渲染器，`<div align="center">x</div>` 会连着标签一起显示（CommonMark 也是把原始 HTML 原样传出），既不渲染也不吞掉。
- **引用不支持「懒惰续行」**：每一行都要自己带 `>`，`> 一行` 后面接一行没写 `>` 的文字会被当成引用外的段落——CommonMark 把它算作同一段引用。
